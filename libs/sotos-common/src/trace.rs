...
//!
//! This module provides trace level definitions, category bitmasks, syscall
//! name lookup, and a small register formatter. NO I/O logic lives here —
//! each consumer (kernel, init, shell) provides its own output backend.

/// Severity / verbosity level for trace messages.
#[repr(u8)]
...
impl TraceLevel {
    ...
    /// Single-character label for prefix output: E / W / I / D / T.
...
/// Trace category bitmask constants.
pub mod cat {
...
/// Return the short name for a single-bit category mask.
pub const fn cat_name_bytes(mask: u16) -> &'static [u8] {
    match mask {
        cat::SYSCALL  => b"SYSCALL",
        cat::IPC      => b"IPC",
        cat::SCHED    => b"SCHED",
        cat::MM       => b"MM",
        cat::FS       => b"FS",
        cat::NET      => b"NET",
        cat::SIGNAL   => b"SIGNAL",
        cat::PROCESS  => b"PROCESS",
        cat::REGISTER => b"REGISTER",
        _             => b"?",
    }
}
...
///
/// Built from every `pub const SYS_*` in `linux_abi.rs`.
...
/// Format `name=0xVAL` into `buf`, returning the number of bytes written.
///
/// Example: `fmt_reg(b"rdi", 0x1000, buf)` writes `b"rdi=0x1000"`.
pub fn fmt_reg(name: &[u8], val: u64, buf: &mut [u8]) -> usize {
    ...
    if val == 0 {
        ...
        while shift >= 0 {
            if pos >= buf.len() { return pos; }
            let nib = ((val >> shift as u32) & 0xF) as u8;
