...
/// Severity level for trace output.
#[repr(u8)]
...
impl TraceLevel {
    /// Single-character tag byte for this level (E/W/I/D/T).
...
/// Map a Linux x86_64 syscall number to its name (lowercase, no `SYS_` prefix).
///
...
pub const fn syscall_name(nr: u64) -> &'static [u8] {
    match nr {
        ...
        151 => b"mlockall",
        152 => b"munlockall",
        154 => b"sched_setaffinity",
        155 => b"sched_getaffinity",
        157 => b"prctl",
        186 => b"gettid",
        200 => b"tkill",
        201 => b"time",
...
/// Format `"  NAME=0x0000000000001234\n"` into `buf` and return bytes written.
///
/// Produces fixed-width 16-hex-digit values so log output aligns neatly.
