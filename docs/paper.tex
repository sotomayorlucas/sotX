...
% --- Title ---
\title{\textbf{sotX: A 4th-Generation Microkernel with Per-Core IPC,\\
...
\begin{abstract}
We present \textbf{sotX} (Secure Object Transactional Operating System), a
...
\textbf{sotX} resolves this tension through a combination of techniques:
\begin{itemize}[nosep]
...
\end{tikzpicture}
\caption{sotX architecture: five kernel primitives, all drivers in userspace.}
\label{fig:arch}
\end{figure}
...
The sotX WASM runtime (\texttt{libs/sotos-wasm/}) is a \texttt{no\_std}
...
Every kernel object in sotX is accessed through \emph{capabilities}:
...
enabling transparent message delivery across a network of sotX nodes:
...
sotX implements defense-in-depth security:
...
sotX v2.0 includes 9 device driver libraries, all running in userspace:
...
sotX demonstrates that a formally-verifiable microkernel can scale to a
...
\begin{itemize}[nosep]
    \item Self-hosting (port a language toolchain to sotX)
