...
The updated test suite now comprises \textbf{147 tests} across the workspace
(was 132), with 12 new tests covering the five features above.  All pass.
...
and 147 Rust tests, \sotfs{} achieves multi-layer verification spanning formal
proofs, model checking, typestate, concurrency testing, and property-based testing.

The immediate next steps are: (1) complete the \textsc{Rmdir} Coq proof by
adding \textsc{NoHardLinkToDir} to \textsc{WellFormed}; (2) write Coq proofs
for cross-directory rename; (3) formalize xattr and symlink invariant preservation;
(4) Verus integration for verified Rust code extraction.
