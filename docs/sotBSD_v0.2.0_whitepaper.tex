...
\hypersetup{
  ...
  urlcolor=linkcolor,
  citecolor=linkcolor,
  pdftitle={sotBSD v0.2.0 -- a verified microkernel BSD personality with deception, SMF, Crossbow and FMA},
...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{\textit{sotBSD v0.2.0 whitepaper}}
...
\begin{abstract}
\noindent
\textbf{sotBSD} is a verified microkernel operating system written from
...
sotBSD enables that no other open-source operating system in 2026 does.
\end{abstract}
...
sotBSD is the BSD personality branch of \texttt{sotOS}, a research
...
\begin{itemize}[leftmargin=1.4em,itemsep=0.3em]
  ...
  \item Real \textbf{NetBSD rump kernel} linked into a freestanding
        sotOS service: \texttt{librump.a + librumpvfs.a +
...
exokernel layer. sotBSD is small enough (108\,k LOC of Rust) to be
...
\subsection{What sotBSD defends against}

sotBSD targets two threat classes that conventional operating systems
handle poorly:
...
sotBSD changes the contract. Every kernel object is accessed through a
...
On sotBSD, the kernel implements \emph{five primitives} and 35-ish
...
\texttt{sotBSD} HEAD \texttt{eb137e3}.
...
\texttt{target/sotos.img} \emph{twice on each host} via
...
(\texttt{if: github.repository == 'sotomayorlucas/sotBSD'}) prevents
fork-tag accidents.
...
sotBSD v0.2.0 ships a verified microkernel BSD personality OS in
...
\href{https://github.com/sotomayorlucas/sotBSD}{\texttt{github.com/sotomayorlucas/sotBSD}}
\quad \textit{Branch:} \texttt{sotBSD}
