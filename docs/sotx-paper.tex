...
\hypersetup{
  ...
  urlcolor=linkcolor,
  citecolor=linkcolor,
  pdftitle={sotX: A Capability-Secure Microkernel with Graph-Structured Filesystem},
...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{\textit{sotX: Capability-Secure Microkernel with Graph-Structured FS}}
...
\title{
  \vspace{-1.5cm}
  \textbf{sotX}: A Capability-Secure Microkernel with \\
...
\begin{abstract}
\noindent
We present \sotx{}, a capability-secure microkernel operating system
...
\sotx{}, a capability-secure microkernel with five primitives
...
Section~\ref{sec:arch} presents the \sotx{} kernel architecture.
...
\sotx{} capability model draws on seL4~\cite{klein2009},
...
The \sotx{} kernel follows the L4 tradition~\cite{liedtke1993}:
...
\sotx{} integrates ideas from all of these: L4-style synchronous
...
The \sotx{} microkernel provides exactly five primitives:

\begin{definition}[Kernel Primitive Set]
\label{def:primitives}
The \sotx{} kernel implements the primitive set
...
\sotx{} ships with seven deception profiles, each defining honeypot files,
tripwire directories, and tripwire filenames:
...
\midrule
ubuntu-web  & Apache/MySQL/PHP & \texttt{.bash\_history}, \texttt{.ssh/authorized\_keys}, \texttt{.env} \\
centos-db   & PostgreSQL/Redis & \texttt{pg\_hba.conf}, \texttt{.pgpass}, \texttt{redis.conf} \\
iot-camera  & Hikvision IP cam & \texttt{config.ini}, \texttt{/etc/passwd} \\
windows-smb & Windows SMB      & \texttt{passwords.xlsx}, \texttt{vpn-credentials.txt} \\
k8s-node    & Kubernetes 1.29  & \texttt{admin.conf}, \texttt{etcd/ca.key}, SA \texttt{token} \\
cicd-runner & GitHub Actions   & AWS \texttt{credentials}, Docker \texttt{config.json}, GH \texttt{hosts.yml} \\
ad-dc       & Active Directory & \texttt{ntds.dit}, \texttt{SAM}, \texttt{krbtgt\_hash.txt}, \texttt{logon.bat} \\
\bottomrule
\end{tabular}
\end{center}

Each access to a tripwire file generates a \texttt{HoneypotAlert} with
severity (Low$\to$Medium$\to$High$\to$Critical) and full forensic context
(\texttt{source\_cap}, \texttt{domain\_id}, \texttt{timestamp}), integrated
into the provenance log for incident response.

A game-theoretic \textbf{indistinguishability analysis}
(\texttt{check\_indistinguishability()}) bounds the attacker's advantage
from three channels: structural (invariant violation), curvature deviation,
and timing side-channel.  Passthrough projections achieve advantage~$= 0$;
fabricated projections with timing calibration achieve advantage~$< 0.3$.
...
The \sotx{} deception framework comprises:
\begin{enumerate}[label=(\roman*)]
...
\sotx{} is implemented entirely in Rust (nightly toolchain,
...
graphs---could benefit from the same framework. The \sotx{} SOT
...
We have presented \sotx{}, a capability-secure microkernel operating
...
The \sotx{} microkernel and \sotfs{} filesystem are open-source and
