...
\hypersetup{
  ...
  urlcolor=linkcolor,
  citecolor=linkcolor,
  pdftitle={sotX System Manual},
  pdfauthor={Lucas Sotomayor},
  pdfsubject={Comprehensive system manual for the sotX microkernel OS},
...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[LE]{\textit{sotX System Manual}}
...
{\Huge\bfseries\color{tnblue} sotX System Manual\par}
\vspace{0.8cm}
{\Large\color{tnblue!70!black} The SOT Exokernel, sotX Microkernel,\\
...
\section{What is sotX?}

\sotX{} is a verified microkernel operating system written from scratch in
...
\sotX{} is structured as a layered system with a minimal microkernel at its
...
\end{tikzpicture}
\caption{sotX architecture stack. The microkernel provides five primitives;
...
a comprehensive Linux system call interface on top of the \sotX{} kernel.
Rather than emulating a Linux kernel, \LUCAS{} translates Linux syscalls
into sequences of \sotX{} IPC calls and local operations within the init
...
\sotX{} service. The rump kernel provides a complete NetBSD kernel
...
\begin{itemize}[leftmargin=1.4em]
  ...
  \item \textbf{Hypercalls:} 60 hand-rolled \texttt{rumpuser\_*} functions
    in \texttt{rumpuser\_sot.c} that bridge the rump kernel to \sotX{}
...
The engine includes seven graph-native deception profiles, each defining
honeypot files, tripwire directories, and tripwire filenames:

\begin{table}[H]
\centering
\caption{Built-in deception profiles (filesystem layer).}
\begin{tabularx}{\textwidth}{l l X}
\toprule
\textbf{Profile} & \textbf{Target} & \textbf{Key Honeypots} \\
\midrule
ubuntu-web  & Apache/MySQL   & \texttt{.bash\_history}, \texttt{.ssh/authorized\_keys}, \texttt{.env} \\
...
Each profile also defines consistent OS-level responses for:
\begin{itemize}[leftmargin=1.4em]
...
generates a \texttt{HoneypotAlert} with severity (Low/Medium/High/Critical)
and full forensic context (\texttt{source\_cap}, \texttt{domain\_id},
\texttt{timestamp}), fed into the provenance log for incident response.

\textbf{Indistinguishability Game.}  A game-theoretic analysis bounds the
attacker's advantage from structural, curvature, and timing channels.
Passthrough projections achieve advantage~$= 0$.
...
\sotFS{} is the native filesystem for \sotX{}, designed around a type graph
...
\caption{Common QEMU flags used with sotX.} \\
\toprule
