...
pub(crate) static LAST_EXEC_PERSONALITY: AtomicU64 = AtomicU64::new(0);
...
fn exec_loaded_elf(file_size: usize, bin_name: &[u8], argv: &[[u8; MAX_EXEC_ARG_LEN]], envp: &[[u8; MAX_EXEC_ARG_LEN]], target_as: u64) -> Result<(u64, u64), i64> {
    ...
    LAST_EXEC_PERSONALITY.store(crate::process::PERS_DEFAULT, Ordering::Release);
    ...
    if let Some(ref interp) = interp_info {
        ...
        if is_glibc {
            LAST_EXEC_PERSONALITY.store(crate::process::PERS_GLIBC, Ordering::Release);
        }
