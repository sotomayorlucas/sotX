...
/// Find a pipe operator `|` outside of single/double quotes. Skips `||` (logical OR).
...
/// Find the first `&&` or `||` outside quotes. Returns (position, is_and).
/// is_and=true for `&&`, is_and=false for `||`.
pub fn find_logical_op_unquoted(s: &[u8]) -> Option<(usize, bool)> {
    let mut i = 0;
    let mut in_single = false;
    let mut in_double = false;
    while i < s.len() {
        match s[i] {
            b'\'' if !in_double => in_single = !in_single,
            b'"'  if !in_single => in_double = !in_double,
            b'&' if !in_single && !in_double && i + 1 < s.len() && s[i + 1] == b'&' => {
                return Some((i, true));
            }
            b'|' if !in_single && !in_double && i + 1 < s.len() && s[i + 1] == b'|' => {
                return Some((i, false));
...
/// Find first `;` outside quotes. Does NOT match `;;` (case/esac pattern separator).
pub fn find_semicolon_unquoted(s: &[u8]) -> Option<usize> {
    let mut i = 0;
    let mut in_single = false;
    let mut in_double = false;
    while i < s.len() {
