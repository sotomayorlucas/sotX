...
#[inline(always)]
pub fn syscall4(nr: u64, a1: u64, a2: u64, a3: u64, a4: u64) -> i64 {
    let ret: i64;
    unsafe { core::arch::asm!("syscall", inlateout("rax") nr => ret,
...
/// wait4 with status pointer and options. Returns child pid, 0 (WNOHANG + still running), or negative error.
#[inline(always)] pub fn linux_waitpid_status(pid: u64, status: *mut u32, options: u32) -> i64 { syscall4(61, pid, status as u64, options as u64, 0) }
/// Non-blocking wait (WNOHANG=1). Returns child pid if exited, 0 if still running.
#[inline(always)] pub fn linux_waitpid_wnohang(pid: u64) -> i64 { syscall4(61, pid, 0, 1, 0) }
/// nanosleep(timespec*) — sleep for given duration
#[inline(always)] pub fn linux_nanosleep(req: *const u8) -> i64 { syscall2(35, req as u64, 0) }
