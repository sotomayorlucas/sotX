...
use super::{
    SYS_DOMAIN_ADJUST, SYS_DOMAIN_ATTACH, SYS_DOMAIN_CREATE, SYS_DOMAIN_DETACH, SYS_DOMAIN_INFO,
};
...
pub fn handle(frame: &mut TrapFrame, nr: u64) -> bool {
    match nr {
        ...
        SYS_DOMAIN_ATTACH => match cap::validate(frame.rdi as u32, Rights::WRITE) {
        ...
        // rdi = domain_cap, rsi = thread_cap; requires WRITE on both
        SYS_DOMAIN_DETACH => match cap::validate(frame.rdi as u32, Rights::WRITE) {
            Ok(CapObject::Domain { id: dom_id }) => {
                match cap::validate(frame.rsi as u32, Rights::WRITE) {
                    Ok(CapObject::Thread { id: tid }) => {
        ...
        SYS_DOMAIN_ADJUST => match cap::validate(frame.rdi as u32, Rights::WRITE) {
            Ok(CapObject::Domain { id: dom_id }) => {
        ...
        SYS_DOMAIN_INFO => match cap::validate(frame.rdi as u32, Rights::READ) {
            Ok(CapObject::Domain { id: dom_id }) => {
