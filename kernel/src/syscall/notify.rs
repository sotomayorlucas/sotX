...
use super::{
    SYS_NOTIFY_CREATE, SYS_NOTIFY_POLL, SYS_NOTIFY_SIGNAL, SYS_NOTIFY_WAIT, SYS_THREAD_NOTIFY,
};
...
pub fn handle(frame: &mut TrapFrame, nr: u64) -> bool {
    match nr {
        ...
        SYS_NOTIFY_CREATE => match notify::create() {
            Some(n) => {
        ...
        SYS_NOTIFY_WAIT => match cap::validate(frame.rdi as u32, Rights::READ) {
            Ok(CapObject::Notification { id }) => match notify::wait(PoolHandle::from_raw(id)) {
                Ok(()) => frame.rax = 0,
        ...
        SYS_NOTIFY_SIGNAL => match cap::validate(frame.rdi as u32, Rights::WRITE) {
            Ok(CapObject::Notification { id }) => match notify::signal(PoolHandle::from_raw(id)) {
                Ok(()) => frame.rax = 0,
                Err(e) => frame.rax = e as i64 as u64,
            },
            Ok(_) => frame.rax = SysError::InvalidCap as i64 as u64,
        ...
        SYS_NOTIFY_POLL => match cap::validate(frame.rdi as u32, Rights::READ) {
