...
use super::{
    SYS_IOPORT_CREATE, SYS_IRQ_ACK, SYS_IRQ_CREATE, SYS_IRQ_REGISTER, SYS_PORT_IN, SYS_PORT_OUT,
};
...
pub fn handle(frame: &mut TrapFrame, nr: u64) -> bool {
    match nr {
        ...
        SYS_IRQ_REGISTER => match cap::validate(frame.rdi as u32, Rights::READ) {
            Ok(CapObject::Irq { line }) => match cap::validate(frame.rsi as u32, Rights::WRITE) {
        ...
        SYS_IRQ_ACK => match cap::validate(frame.rdi as u32, Rights::READ) {
            Ok(CapObject::Irq { line }) => match irq::ack(line as u8) {
                Ok(()) => frame.rax = 0,
                Err(e) => frame.rax = e as i64 as u64,
            },
            Ok(_) => frame.rax = SysError::InvalidCap as i64 as u64,
        ...
        // Width defaults to 1 for any value other than 2 or 4 (backward compatible).
        SYS_PORT_OUT => match cap::validate(frame.rdi as u32, Rights::WRITE) {
            Ok(CapObject::IoPort { base, count }) => {
                let port = frame.rsi as u16;
